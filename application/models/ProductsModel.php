...
class ProductsModel extends CI_Model
{
 ...
 public function fetch_details($limit, $start, $category)
 {
  ...
  $this->db->where(["is_deleted" => 0, 'out_stock' => 0]);
  if ($category != '') {
 ...
 {
    if ($id) {
      $query = $this->db->select("CONCAT('".images('products/')."', image) image, id, eng_name,guj_name, price, min_qty, qty_type, qty, slug")
            ->from("cart c")
            ->where(["is_deleted" => 0, 'out_stock' => 0])
      ...
      $query = $this->db->select("CONCAT('".images('products/')."', image) image, id, eng_name,guj_name, price, min_qty, qty_type, slug")
            ->from("products p")
            ->where(["is_deleted" => 0, 'out_stock' => 0])
            ->where_in('id', $get)
            ->order_by("eng_name", "ASC")
 ...
 public function total($id)
  ...
  {
    $orders = $this->db->select("id, total_amount, order_details, created_at, status, payment_status, payment_id, delivery_address address")
                    ->where(['id' => $id])
