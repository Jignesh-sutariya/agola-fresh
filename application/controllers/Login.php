...
class Login extends CI_Controller {
    ...
    public function index()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'GET'){
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                $id = $this->main->get($this->table, 'id user_id, fullname,mobile,address,pincode', $post);
                if ($id) {
    ...
    public function signup()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'GET'){
            ...
            $this->form_validation->set_rules('mobile', 'Mobile', 'required|is_unique[customers.mobile]|numeric|exact_length[10]', ['required' => 'Enter %s.','is_unique' => '%s Already Registered.','numeric' => 'Enter Valid %s.','exact_length' => 'Enter Valid %s.']);
